...
func (s *FileServer) stream(msg *Message) error {
	...
	mw := io.MultiWriter(peers...) // Peer -> net.Conn also implements the Reader/Writer
...
func (s *FileServer) Get(key string) (io.Reader, error) {
	if s.store.Has(key) {
		fmt.Println("[%s] serving file (%s) from local disk", s.Transport.Addr(), key)
		return s.store.Read(key)
	}

	fmt.Printf("[%s] don't have file (%s) locally, fetching from network...\n", s.Transport.Addr(), key)

	// search over in the network
	msg := Message{
...
func (s *FileServer) handleMessageGetFile(from string, msg MessageGetFile) error {
	...
	// First send the incomingStream byte to peer and then we can send the file size
	// as an int64.
	peer.Send([]byte{p2p.IncomingStream})
